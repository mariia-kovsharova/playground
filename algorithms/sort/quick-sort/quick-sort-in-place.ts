 ...
 */
const quickSortInPlace = <T>(array: Array<T>): Array<T> => {
    ...
    */
    ...
    /* 
        @param originalArray - the collection of items that we need to sort
    ...
    */
    const inner = (
        ...
        isRecursive: boolean = false): Array<T> => {
        ...
        if (start < end) {
            // here we get the partition index which "slice" the array
            // at two parts
            const partitionIndex = partition(collection, start, end);

            inner(collection, start, partitionIndex - 1, true);
            inner(collection, partitionIndex + 1, end, true);
        }
