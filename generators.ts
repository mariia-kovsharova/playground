...
const it2 = gen(5); // <== получение генератора
it2.next(); // <== запуск генератора
it2.next(5); // <== передача в генератор стартового значения последовательности

console.log(it2.next()); // 10
console.log(it2.next()); // 15
console.log(it2.next()); // 20

it2.return(); // принудительное завершение генератора - т.к. он бесконечный, посылаем сигнал об окончании работы
